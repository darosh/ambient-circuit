		...
		if (bitDepth === 16) {
			for (let i = 0; i < totalSamples; i++) {
				...
				for (let c = 0; c < channels; c++)
					s += view.getInt16(dataOffset + (i * channels + c) * 2, true)
			...
			for (let i = 0; i < totalSamples; i++) {
				...
				mono[i] = s / channels / 8_388_608
			}
		} else return null

		return detectLoop(mono, sampleRate)
	} catch {
		return null
	}
}
...
export function detectNonWavLoop(filePath: string, sampleRate: number) {
	...
	const decoded = decodeWithFfmpeg(filePath, sampleRate)
	if (!decoded) return null
	return detectLoop(decoded.samples, sampleRate)
}

/**
 * Decode audio file to mono float32 PCM via ffmpeg.
 * Returns { sampleRate, channels, samples: Float32Array } or null.
 */
function decodeWithFfmpeg(filePath: string, targetSR = 44100) {
	try {
		const buf = execSync(
			`ffmpeg -i "${filePath}" -f f32le -ac 1 -ar ${targetSR} pipe:1 2>/dev/null`,
			{ maxBuffer: 64 * 1024 * 1024 }
		)
		const samples = new Float32Array(buf.buffer, buf.byteOffset, buf.byteLength / 4)
		return { sampleRate: targetSR, channels: 1, samples }
	} catch {
		return null
	}
}
