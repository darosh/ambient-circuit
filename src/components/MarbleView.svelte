<script lang="ts">
	...
	const sides = $derived(marble.runtime.sides ?? marble.resolved.sides ?? 6)
	const rounds = $derived(marble.runtime.rounds ?? marble.resolved.rounds ?? 3)
	const angle = $derived(marble.runtime.angle ?? marble.resolved.angle ?? 60)
