...
export function sharedInstrumentMaterial() {
	...
	mat.outputNode = Fn(() => {
		...
		const effect = noise1
			.mul(noise2)
			.mul(fadeFactor)
			.mul(impactIntensity.add(0.7 * 0.8)) // baked default initialIntensity

		const emissiveColorLuminance = luminance(emissiveColor)
		const baseColor = emissiveColor.mul(impactIntensity.add(0.7)).div(emissiveColorLuminance)
	...
	function setUvMax(uvMax: number) {
		uvFreqEffective.value = uvMax > 0 ? Math.max(1, Math.round(uvMax * 0.1)) / uvMax : 0.1
		// Note: if uvFreq varies per instrument, store it in mesh.userData and update here instead
	}
	...
	return {
		...
		impactIntensity,
		activeUniform,
		uvFreqEffective, // expose if you need to tweak globally or per-mesh
		useFadeUniform,
		setUvMax // can call globally, or make per-mesh if needed
	}
}
