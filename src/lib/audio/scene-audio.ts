...
export async function buildSceneAudio(
	...
	for (const ie of sceneCtx.instruments) {
		if (ie.instrument.audio) {
			if (ie.instrument.audio.id && engine.chains.has(ie.instrument.audio.id)) {
				ie.audio = engine.chains.get(ie.instrument.audio.id)
			} else {
				ie.audio = await buildChain(engine, ie.instrument.audio, defaultAnalyser)
			}
		}
	}
	...
	for (const rd of rails) {
		...
		for (const md of mds) {
			if ('audio' in md && md.audio) {
				...
				if (me) {
					if (md.audio.id && engine.chains.has(md.audio.id)) {
						me.audio = engine.chains.get(md.audio.id)
					} else {
						me.audio = await buildChain(engine, md.audio, defaultAnalyser)
					}
				}
			}
			mIdx++
